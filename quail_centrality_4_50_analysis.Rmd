...
full.outputs$scld.postXpre.str = full.outputs$scld.post.str - full.outputs$scld.for.str
...
outputs = c("scld.forXpre.str", "props.for", "scld.postXfor.str", "scld.postXpre.str", "med.run.energy") #probably also want plots for proportion of times producer was remembered as fss
...
full.outputs %>% 
              filter(attention == 1,
                     ...
                     groupsize == 15) %>%
              data_summary(varname = "forXpre.str", 
                          groupnames = c("med.run.energy", "mem", "approachfood")) %>%
              ggplot( aes(x=med.run.energy, y=median, group=as.factor(mem), color=as.factor(mem))) + 
                  #geom_line() +
                  geom_point()+
                  facet_grid(cols = vars(approachfood))+
                  #geom_errorbar(aes(ymin=lowquant, ymax=highquant), width=.05) +
                  labs(title=paste0("Effect of ", "med.run.energy", " on median ", "forXpre.str"), 
                       x = "med.run.energy", 
                       y = "forXpre.str")+
                  theme_classic()
