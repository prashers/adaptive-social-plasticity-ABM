...
for(i in group.sizes) {
  ...
  loop.data = bind_cols(loop.data, split.fsslist) #bind_cols is the same as cbind but faster
  ...
  for(j in unique(loop.data$run)) {
    ...
    for(k in agents){
      ...
      loop.data2 = bind_cols(loop.data2, change.log, .name_repair = "unique") #unique name_repair will give each column a unique name
      
      #names(loop.data2)[ncol(loop.data2)] = paste0("change.log.", k+1) #change the name of the last column
      ...
      loop.data2 <- loop.data2 %>%
                      mutate(change = as.numeric(select(., last_col()) == TRUE))
