...
#there should be 9000 unique combinations of the 7 variables I allowed to vary (5*9*5*5*2*2*2)
num.combos = 9000
...
nrow(data.ord[data.ord$groupsize==3 & 
                  ...
                  data.ord$approachfood=="false" &
                  data.ord$resetfoodconsistent=="false" &
                  data.ord$resizearena=="false",]) # every combo has 15050 rows

data.ord[rownames(data.ord) == 1505, colnames(data.ord) %in% c("groupsize", "mem", "attention", "preference", "approachfood", "resetfoodconsistent", "resizearena")] #checking whether combo changes every 15050 rows
...
write_csv(resets.summary, "C:/Users/sanja/Documents/Sanjay's stuff/QuailCentralityABM/R analyses/quail_centrality_4/5percombo/resets_summary.csv")
...
data.ord = data.ord[ , !(names(data.ord) %in% c("resetnum", "depletenum"))]

write_csv(data.ord, "C:/Users/sanja/Documents/Sanjay's stuff/QuailCentralityABM/R analyses/quail_centrality_4/5percombo/data_ord_5percombo.csv") #saving csv here so I can run the big loop after restarting R with clean memory
...
for (i in group.sizes) {
  ...
  loop.data = read_csv_chunked("C:/Users/sanja/Documents/Sanjay's stuff/QuailCentralityABM/R analyses/quail_centrality_4/5percombo/data_ord_5percombo.csv", callback = DataFrameCallback$new(f))
  ...
  data.split = cbind(loop.data[, !(names(loop.data) %in% c("proximIDs"))], split.prox) #, split.foll) #, split.cor) #add the new columns into the dataframe with proxim.IDs column removed
  #View(q.data.split)
  if(i==3){
...
rm(data.ord, split.prox, split.proximIDs, x, loop.data)
...
for(i in group.sizes){
  ...
  #data.split = as.data.frame(data.split[,-1])
  ...
  rm(data.pre) # remove q.data.pre from environment to save memory
...
#  unique(prox.count.pre$prox.key)# check that all NAs were replaced
  ...
  prox.count.pre = prox.count.pre %>% 
    group_by(run, mem, attention, preference, approachfood, prox.key, prox.ID) %>% 
    summarize(n = n()) 
...
#  #min(grep("0", data.forage$current.succ.foragers, fixed=T)) # shows numerical index of all the rows where current.succ.foragers contains a zero - the minimum value within each model run should be the first time the producer accessed the food patch
  ...
  df.fed = data.frame()
#  n.loops = max(unique(data.forage$run.num))
#  pb = txtProgressBar(min = min(data.forage$run.num), max = n.loops, style=3)
#  start.time = Sys.time()
  
  for (j in unique(data.forage$run.num)) {
#    setTxtProgressBar(pb,j)#update progress bar
    
    mod.run = data.forage[data.forage$run.num==j,] # subset with data from one model run
    ...
    df.fed = rbind(df.fed, mod.run.fed) # save subset in external dataframe  
...
#  run.time #ran in 4 minutes for group of 20
  ...
  file.name = paste(file.path, "df_fed.csv", sep = "/")
  write_csv(df.fed, file.name)
  
  
  prox.count.for = df.fed %>% 
    pivot_longer(starts_with("prox"), #pivot_longer is the same as melt in the reshape2 package
                 names_to = "prox", values_to = "prox.ID") 


  #nrow(df.fed)*(i*(i-1)) == nrow(prox.count.for)#check that prox.count has the correct number of rows

  prox.count.for = tibble::add_column(prox.count.for, prox.key = "NA", .after = "prox")

  rm(data.forage,df.fed) # remove data.forage from environment to save memory
...
###PROXIMITY network: POST-foraging phase###  
  ...
  rm(data.post) # remove q.data.forage from environment to save memory
...
##### NOW ALL EDGE LISTS CAN BE USED IN THE "quail_centrality_4_analysis5" R SCRIPT
```
